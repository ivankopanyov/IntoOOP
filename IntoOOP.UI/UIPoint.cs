...
public readonly struct UIPoint
{
    ...
    public static UIPoint operator +(UIPoint p1, UIPoint p2) => new UIPoint(p1.X + p2.X, p1.Y + p2.Y);
    ...
    public static UIPoint operator *(UIPoint p, int n) => new UIPoint(p.X * n, p.Y * n);
    ...
    public static UIPoint operator /(UIPoint p, int n)
    {
        if (n == 0) throw new DivideByZeroException();
        return new UIPoint(p.X / n, p.Y / n);
    }
